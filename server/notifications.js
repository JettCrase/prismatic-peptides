...
function orderText(order, source = 'payment') {
  ...
  return [
    `Prismatic Peptides order backup`,
    `Source: ${source}`,
    `Order #: ${order.id}`,
    `Status: ${order.status}`,
...
function orderHtml(order, source = 'payment') {
  ...
  return `
    <h2>Prismatic Peptides order backup</h2>
...
async function sendEmailBackup(order, source) {
  ...
  await transport.sendMail({
    ...
    subject: `Prismatic Peptides Order #${order.id} - ${money(order.total)}`,
