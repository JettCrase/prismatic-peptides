...
function paymentHTML(order) {
  ...
  if (provider === 'paypal') {
    return `<span class="admin-payment admin-payment-legacy">Legacy external checkout</span>${order.paypal_order_id ? `<br><span class="admin-muted">${escapeHtml(order.paypal_order_id)}</span>` : ''}`;
  }
  if (provider === 'manual_paypal') {
    return `<span class="admin-payment admin-payment-legacy">Legacy external manual</span><br><span class="admin-muted">Match exact total</span>`;
  }
  if (provider === 'crypto') {
...
function paymentProviderGroup(order) {
  ...
  if (provider.includes('paypal')) return 'legacy_external';
